...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      EXECUTE 'DROP POLICY IF EXISTS "' || policy_record.policyname || '" ON public.groups';
    END LOOP;
  END;
END $$;
...
BEGIN
  ...
  BEGIN
    FOR policy_record IN 
      SELECT policyname 
      FROM pg_policies 
      WHERE tablename = 'group_join_requests' AND schemaname = 'public'
    LOOP
      EXECUTE 'DROP POLICY IF EXISTS "' || policy_record.policyname || '" ON public.group_join_requests';
    END LOOP;
  END;
END $$;
...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      EXECUTE 'DROP POLICY IF EXISTS "' || policy_record.policyname || '" ON public.group_messages';
    END LOOP;
  END;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'trigger_update_group_updated_at'
  ) THEN
